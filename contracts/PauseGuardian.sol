...
contract PauseGuardian is Ownable2Step, AutomationCompatible {
    ...
    function pauseMintingAndBorrowingForMarket(address cToken) external onlyOwner {
        comptroller._setMintPaused(cToken, true);
        comptroller._setBorrowPaused(cToken, true);
    }

    function pauseMinting(address cToken) external onlyOwner {
        comptroller._setMintPaused(cToken, true);
    }

    function pauseBorrowing(address cToken) external onlyOwner {
        comptroller._setBorrowPaused(cToken, true);
    }
    ...
    function _canPause() internal view returns (bool) {
        ...
        for (uint i; i < marketCount; ++i) {
            ...
            ProofOfReserveFeed proofOfReserveFeed = ProofOfReserveFeed(proofOfReserveFeeds[cTokenAddress]);

            uint underlyingTokenTotalSupply = UnderlyingToken(CToken(cTokenAddress).underlying()).totalSupply();
    ...
    function setProofOfReserveFeed(address cToken, address feed) external onlyOwner {
        _setProofOfReserveFeed(cToken, feed);
    }

    function _setProofOfReserveFeed(address cToken, address feed) internal {
        if (proofOfReserveFeeds[cToken] == address(0)) {
            markets.push(cToken);
        }

        proofOfReserveFeeds[cToken] = feed;
    }

    function removeProofOfReserveFeed(address cToken) external onlyOwner {
        delete proofOfReserveFeeds[cToken];
