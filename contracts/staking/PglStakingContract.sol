...
contract PglStakingContract is ReentrancyGuard, PglStakingContractStorage {
    ...
    /**
     * Deposit Pangolin PROTOCOL-NATIVE liquidity provider tokens into the staking contract.
     *
    ...
    function claimRewards() external nonReentrant {
        ...
        uint amount = accruedReward[msg.sender][REWARD_PROTOCOL];
        claimErc20(REWARD_PROTOCOL, msg.sender, amount);
    }
    ...
    function getClaimableRewards(uint rewardToken) external view returns(uint) {
        ...
        // Static reward value for native token due to erroneously emitted native token
        if (rewardToken == REWARD_NATIVE) {
            return 0;
        }
    ...
    /**
     * Fallback function to accept native deposits.
     */
    ...
    function setRewardTokenAddress(uint rewardToken, address rewardTokenAddress) external adminOnly {
        require(rewardToken != REWARD_NATIVE, "Cannot set native address");
        rewardTokenAddresses[rewardToken] = rewardTokenAddress;
    }

    /**
     * Set PROTOCOL-NATIVE PGL token contract address.
     *
     * @param newPglTokenAddress New PROTOCOL-NATIVE PGL token contract address
     */
    ...
    /**
     * Transfer native token rewards from the contract to the reward recipient.
     *
     * @param recipient Address, whose native token rewards are claimed
     * @param amount The amount of claimed native tokens
     */
    function claimNativeToken(address payable recipient, uint amount) internal {
        require(accruedReward[recipient][REWARD_NATIVE] <= amount, "Not enough accrued rewards");

        accruedReward[recipient][REWARD_NATIVE] = accruedReward[recipient][REWARD_NATIVE].sub(amount);
    ...
    function claimErc20(uint rewardToken, address recipient, uint amount) internal {
        require(rewardToken != REWARD_NATIVE, "Cannot use claimErc20 for native token");
