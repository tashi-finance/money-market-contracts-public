...
import "./GenesisPoolStakingContractProxyStorage.sol";

contract GenesisPoolStakingContractStorage is GenesisPoolStakingContractProxyStorage {
    // CToken contract address for this genesis pool
    address public genesisPoolCTokenAddress;

    // Addresses of the ERC20 reward token
    ...
    mapping(address => uint) public accruedReward;

    // Supplied CToken tokens per user
    mapping(address => uint) public supplyAmount;

    // Sum of all supplied CToken tokens
    uint public totalSupplies;

    /*
     * rewardIndex keeps track of the total amount of rewards to be distributed for
     * each supplied unit of CToken tokens. When used together with supplierIndex,
     * the total amount of rewards to be paid out to individual users can be calculated
     * when the user claims their rewards.
     *
     * Consider the following:
     *
     * At contract deployment, the contract has a zero CToken balance. Immediately, a new
     * user, User A, deposits 1000 CToken tokens, thus increasing the total supply to
     * 1000 CToken. After 60 seconds, a second user, User B, deposits an additional 500 CToken,
     * increasing the total supplied amount to 1500 CToken.
     *
     ...
     * amount of 1000 CToken tokens at the set reward speed. Assuming a reward speed of
     * 5 tokens per second (denoted 5 T/s), the accrueRewards function calculates the
     * accrued reward per supplied unit of CToken tokens for the elapsed time period.
     * This works out to ((5 T/s) / 1000 CToken) * 60 s = 0.3 T/CToken during the 60 second
     * period. At this point, the global reward index variable is updated, increasing
     * its value by 0.3 T/CToken, and the reward accrual block timestamp,
     ...
     * for which the reward accrued per unit of CToken is ((5 T/s) / 1500 CToken) * 30 s = 0.1 T/CToken.
     * The reward index is updated to 0.4 T/CToken (0.3 T/CToken + 0.1 T/CToken) and the reward
     ...
     * T tokens that have been accrued per supplied CToken during the time since the user's
     * last claim. User A has a supply balance of 1000 CToken, thus having an unclaimed
     * token amount of (0.4 T/CToken - 0 T/CToken) * 1000 CToken = 400 T. This amount is
     ...
     * the form of (0.4 T/CToken - 0.3 T/CToken) * 500 CToken = 50 T. As expected, the total amount
    ...
    // The rewards index for each supplier as of the last time they accrued reward
    mapping(address => uint) public supplierRewardIndex;
    
    // Last time rewards was accrued
    uint public accrualBlockTimestamp;
    
    // Deposit start time, after this time, the GenesisPool will start allow deposits
    uint public depositStartTimestamp;
    
    // Deposit end time, after this time, the GenesisPool will become withdrawal-only
    uint public depositEndTimestamp;
}
